...
- **Fluent Bit Journald Collector** (`configuration/otel/JournaldOtelManifestBuilder.kt`) — a dedicated Fluent Bit DaemonSet (`fluent-bit-journald`) isolated from the main OTel collector. Has a native systemd input plugin with journalctl compiled in. Reads journal files directly from `/var/log/journal` (mounted read-only). Collects all journal entries (no filtering). Uses a `modify` filter to rename `HOSTNAME` → `host.name` and add `source: journald` for unified search. Maps `MESSAGE` to OTLP body (`_msg` in VictoriaLogs) and `PRIORITY` to OTLP severity. Forwards logs via OTLP HTTP to the main collector on `localhost:4318`. Image: `fluent/fluent-bit:latest`.
- **Tool runner logs** — commands run via `exec run` use `systemd-run` transient units (`edl-exec-*`). Output goes to the systemd journal (default behavior) where the Fluent Bit journald collector picks it up with accurate timestamps. Supports foreground and background (`--bg`) execution. Foreground reads output via `journalctl --unit=edl-exec-{name}`.
