...
- **Core dashboards**: JSON files in the top-level `dashboards/<folder>/` tree, one subdirectory per Grafana folder (`cassandra`, `infrastructure`, `observability`, `opensearch`). `GrafanaDashboardCatalog` discovers every file on the classpath at runtime; `grafana update-config` copies the whole tree over SSH onto the control node's Grafana hostPath (`/mnt/db1/grafana/dashboards`, `/var/lib/grafana/dashboards` inside the pod), and a single Grafana file provider with `foldersFromFilesStructure` turns each directory into a folder. Dashboards are never K8s objects and nothing enumerates them by hand. Adding a dashboard is dropping a JSON file into a folder directory; adding a folder is making a directory. The folder name is the directory name verbatim, so a core dashboard for an engine that also has a kit lands in the same folder as the kit's own dashboards. `system-overview.json` must exist somewhere in the tree (it is the home dashboard; discovery fails without it). **Do NOT add kit dashboards here** — they belong with their kit.
