...
- **A broken kit is ALWAYS a P0, major problem.** If a kit fails to install, start, or come up healthy — a dead helm repo, a CrashLooping pod, an image that won't pull, a config the deployed version rejects — that is a **P0** to fix, never a footnote, never "out of scope," never deferred to backlog. This tool's entire purpose is running these workloads; a kit that doesn't work means the tool is broken for that workload. File it P0 and fix the root cause. Do not downgrade, soft-pedal, or wave off a broken kit as "pre-existing" or "unrelated" — surface it as the major problem it is.
