- **This is a testing and fast-iteration tool, not a production tool.** easy-db-lab exists to spin up throwaway clusters and iterate quickly on testing Cassandra builds and fixes.  Optimize for iteration speed and simplicity.  Do not gold-plate features with production hardening the tool does not need — credential rotation, key revocation on live hosts, audit trails, and similar — when it adds friction without value; the clusters are ephemeral and single-operator.  This never licenses leaking secrets or disabling functionality (see the rules below); it means not building production concerns this tool does not have.
...
- **ABSOLUTE RULE: NEVER set, clear, or otherwise touch the `socksProxyHost` / `socksProxyPort` JVM system properties.** Not even temporarily with save/restore. These are JVM-global and capture **every** socket in the process — including the AWS SDK — routing it through the SOCKS tunnel. This caused a major incident and was deliberately removed in #725 (see the commit message and the `Socks5ProxySelector` KDoc). The proxy publishes **only** its port under `Constants.Proxy.PORT_PROPERTY`; cluster clients opt into the tunnel **explicitly and per-client** — Fabric8 via `config.httpsProxy = "socks5://127.0.0.1:{port}"` (`KubernetesClientFactory`), cluster HTTP via `Socks5ProxySelector` installed on that client only (`ProxiedHttpClientFactory`), the CQL driver via `SocksProxyNettyOptions`, and the JDBC `sql` command plus local kubectl/helm in kit shell steps via `SocksTcpBridge` (a per-connection localhost→SOCKS TCP forwarder; see `KubeconfigProxyResolver`). To route a **new** client type through the tunnel, give **that connection** an explicit `java.net.Proxy(SOCKS, ...)` / driver-native SOCKS option (or route it through `SocksTcpBridge`) — never a global property. This invariant is also encoded in the `SOCKS Proxy` requirement in `openspec/specs/networking/spec.md`.
- **ABSOLUTE RULE: NEVER delete the owner's data automatically.** Test results, observability data (metrics, logs, traces, profiles, annotations), backups, and exports are the owner's work product. No code, default, lifecycle rule, retention setting, compactor setting, TTL, or cleanup step may delete them unless the owner runs an explicit delete. This covers S3 lifecycle and expiry rules, `--retention-days`-style options, backend retention such as Tempo `block_retention` or Mimir and Loki compactor retention, and teardown cleanup. Ephemeral clusters are disposable; the data they produce is not. Never add automatic deletion as a feature, a default, or a "cost" optimization, and flag any existing automatic deletion as a critical bug.
- When we are working on a PR, and you find a bug, it should generally be fixed in that patch, NOT deferred. I am not interested in separate patches for bug fixes. If you find a bug, tell me about it, the default behavior should be to fix it.
- During the grilling session, ask one question at a time.
- During reviews, flag any loosening of detekt rules as MAJOR issues.
...
**The `/easy-db-lab:run` skill creates the workspace** — the directory, an `easy-db-lab` wrapper that
`cd`s into it, and the lab-report scaffold — before it executes any plan step. Test plans must **not**
create the workspace themselves. A plan's first step is provisioning (`init ... --up`); everything
after it uses `$EDB`, which `run` sets:
...
For multi-DC plans `run` creates one wrapper per DC and sets `$EDB_DC1`, `$EDB_DC2`, and so on.
...
The project uses **Java 21** (Temurin) as the default — managed via SDKMAN. The build is
JDK-version-flexible: it compiles with whatever JDK is installed (21 or newer, e.g. 25) and
always targets Java 21 bytecode (`jvmTarget`/`release = 21`, no fixed `jvmToolchain`), so
artifacts still run on older JVMs. Java 21 remains the recommended local default.

**Caveat — static analysis needs JDK 21:** `detekt` 1.23.8 (the last stable release) cannot
run under a JDK 25 runtime, so `./gradlew detekt` and the full `./gradlew check` must be run
on JDK 21. Compiling, running, and testing the app all work on JDK 25. CI runs `check` on
JDK 21. Upgrading detekt to the JDK-25-capable 2.0.x line is a separate migration (new
`dev.detekt` coordinate, K2 engine, baseline regeneration).
...
**Collectors** (run on cluster nodes): OTel Collector, Fluent Bit (journald), Grafana Alloy (eBPF profiling), Beyla (L7 RED metrics), ebpf_exporter (TCP/block I/O/VFS), YACE (CloudWatch), kube-state-metrics (K8s object state, control node, scraped by node-local pod discovery), OpenTelemetry Java agent (Cassandra metrics and application logs, over OTLP; the collector tails only the JVM GC log from disk, as `source="cassandra-gc"`), Cilium agent/operator/Hubble scrapes (Cilium clusters only)

**Dynamic OTel config**: `OtelManifestBuilder.buildConfigMap()` accepts `List<WorkloadScrapeConfig>` (plus the telemetry redirect and the `CniMode`) and injects one Prometheus scrape job per running kit. Kits register by writing `easydblab-metrics-<kit>` ConfigMaps (label: `easydblab.com/workload-metrics=true`). `MetricsRegistryService` creates/deletes these ConfigMaps; `OtelSyncService` reads them all and regenerates the OTel collector ConfigMap. Both are called automatically by `KitRunnerCommand` on successful `start`/`stop`.

**Storage backends** (control node): Mimir 3.2.1 (metrics, port 9009, Prometheus API under `/prometheus`), Loki 3.7.8 (logs, port 3100), Tempo 3.0.3 (traces, port 3200), Pyroscope 2.3.1 (profiles, port 4040). Mimir runs no compactor or store-gateway; Loki runs `target: all` with its compactor idle. Both write to S3 as they run, and `down` flushes both (`TeardownFlushService`: annotation mirror → Loki flush → Mimir flush → annotations backup). Any failed step stops `down` with nothing torn down and no backend started again — `down` never scales Loki or Mimir up or replaces their pods (owner decision, 2026-09-26); the failure (`FlushStepFailed`) names the step and each backend's state. A successful flush is recorded as `ClusterState.tailFlush`, so a re-run after a failed teardown skips it; `up` clears it. Readers go through `ObservabilityHttp` (per-client SOCKS unless Tailscale, plus the tenant header); `MimirQueryService`, `LokiQueryService` and `LogQl` build on it. Every Grafana annotation is mirrored to Loki (`AnnotationMirror`).

**Observability store and tenant**: every cluster has a tenant (`init --tenant`, default `default`, `InitConfig.tenant`, read with `ClusterState.tenant()`). All observability data lands in the account bucket, laid out by `ObservabilityStore` (`configuration/`): Mimir's blocks at `observabilitymetrics` (letters and digits only), Loki's at `observability/logs`, Tempo's at `observability/traces` and Pyroscope's at `observability/profiles` (all native multi-tenancy — they make the tenant directories), and annotation backups at `observability/annotations/<tenant>/<yyyyMMdd-HHmmss>_<name>-<clusterId>.json` (`SnapshotName`). The tenant is published in the `cluster-config` ConfigMap (`TENANT`); every writer and reader sends it in `X-Scope-OrgID`, and every Grafana datasource sends it on every query. Nothing deletes observability data: `down` sets no S3 lifecycle rule, the cluster IAM role is denied deletes on the metrics, logs, traces and annotations prefixes, Mimir runs no compactor, Loki's compactor is idle with retention and deletion off, Tempo runs with compaction (and so retention) off, Pyroscope v2 with retention cleanup off, and the JFR shipper never deletes an unshipped or rejected chunk (it stops recording at its byte bound instead). The observability stack writes nothing to the per-cluster data bucket.

**Restarts by config hash**: `ObservabilityStackService.deploy()` never force-restarts workloads. `ConfigHashAnnotator` stamps each workload's pod template with a hash of the rendered pod template (minus the hash annotation, keys sorted) and of the ConfigMaps it reads (including `cluster-config`), so Kubernetes rolls exactly the workloads whose template or configuration changed, and `ConfigChangeReport` reports "unchanged" only when Kubernetes will not roll the workload.
...
- `grafana update-config` recopies the whole tree from `build/resources/main/`, so an edit that skips `./gradlew installDist` silently redeploys the previous build's copy and still reports success.
...
**CLI commands**: `grafana update-config`, `grafana annotate`, `grafana backup`, `logs query`
