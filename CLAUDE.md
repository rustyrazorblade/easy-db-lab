...
- **Journald OTel Collector** (`configuration/otel/JournaldOtelManifestBuilder.kt`) — a dedicated OTel collector DaemonSet (`otel-journald`) isolated from the main collector. Reads systemd journal entries via chroot (`root_path: /host` with host `/` mounted read-only). Filters to `edl-exec-*` units (tool-runner). Forwards logs via OTLP to the main collector on `localhost:4317`. Isolation ensures journald receiver failures (alpha stability) don't affect existing log/metric/trace collection.
- **Tool runner logs** — commands run via `exec run` use `systemd-run` transient units (`edl-exec-*`). Output goes to the systemd journal (default behavior) where the journald OTel collector picks it up with accurate timestamps. Supports foreground and background (`--bg`) execution. Foreground reads output via `journalctl --unit=edl-exec-{name}`.
