...
- Triggering on PRs or nightly builds
...
Dashboard releases are tied to versioned releases via `v*` tags, keeping the release asset stable and correlated with the container image release. Additionally, pushes to `main` update a rolling `latest` pre-release — mirroring the container image's `latest` tag pattern — so users can always get the newest dashboards without waiting for a version tag. A `workflow_dispatch` trigger is also included so the workflow can be tested manually from the Actions UI.

Alternative: Only trigger on version tags. Rejected — the container image publishes `latest` on main, so dashboards should follow the same pattern for consistency.
