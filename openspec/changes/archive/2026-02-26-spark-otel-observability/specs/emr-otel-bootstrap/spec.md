...
The system SHALL upload a bootstrap action script to S3 and configure EMR cluster creation to run it, installing the OTel Java agent JAR (`/opt/otel/opentelemetry-javaagent.jar`), the OTel Collector binary (`/opt/otel/otelcol-contrib`), and the Pyroscope Java agent JAR (`/opt/pyroscope/pyroscope.jar`) on every EMR node, and start the OTel Collector as a systemd service.
