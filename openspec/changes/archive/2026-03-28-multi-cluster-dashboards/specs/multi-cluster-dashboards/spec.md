...
### Requirement: Dead ClickHouse native datasource is removed

The `grafana-clickhouse-datasource` provisioned in `GrafanaDatasourceConfig.kt` SHALL be removed. All dashboard panels use the VictoriaMetrics (prometheus) datasource; no panel references the native ClickHouse datasource.

#### Scenario: ClickHouse native datasource is not provisioned

- **WHEN** Grafana starts and loads its datasource provisioning
- **THEN** no native ClickHouse datasource (type `grafana-clickhouse-datasource`) SHALL be provisioned
