...
### Requirement: install.yaml declares lifecycle phases as typed step sequences
`install.yaml` SHALL support four optional top-level lifecycle keys (`install`, `start`, `stop`,
...
#### Scenario: Lifecycle phases parsed from install.yaml
- **WHEN** `install.yaml` contains an `install` key with a list of typed steps
...
- **WHEN** `install.yaml` omits the `stop` phase
- **THEN** `easy-db-lab <workload> stop` falls back to the `bin/stop.sh` script if present
...
### Requirement: install.yaml declares workload version and dashboard files
`install.yaml` SHALL support:
- A top-level `version` field (string) identifying the workload version being installed
...
successfully, replacing the hardcoded directory scan in `WorkloadRunnerCommand`.

#### Scenario: Dashboards installed after successful start
- **WHEN** `install.yaml` declares a `dashboards` list and `easy-db-lab <workload> start` exits
  successfully
...
`collisionCheck` in `install.yaml` SHALL accept either a boolean (applies to `start` only,
...
emit a `Event.Workload.StepFailed` event with the step type and error detail, and exit with a
non-zero exit code.
...
- **AND** an error event is emitted identifying the step type, phase, and workload name
