# Uninstalled Workload Hint

## Overview

When the user types a top-level subcommand that is not installed but matches an available
workload template, the CLI SHALL print an actionable error message instead of the generic
PicoCLI unmatched-argument error.

## Requirements

### Requirement: Hint for uninstalled available workload

When an unrecognized top-level argument exactly matches the name of an available (installable)
workload template, the CLI SHALL print a targeted message identifying the workload as not
installed and providing the exact command to install it.

#### Scenario: User types an available but uninstalled workload name

- **WHEN** the user runs `easy-db-lab presto` and `presto` is not installed
- **AND** `presto` is a known installable workload template
...
- **AND** `unknownxyz` does not match any available workload template
