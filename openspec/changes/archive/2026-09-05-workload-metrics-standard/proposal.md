...
### New Capabilities
- `kit-metrics-catalog`: Global `bin/export-workload-metrics` script; committed `metrics-catalog.json`, `METRICS.md`, and `dashboards/<workload>.json` artifacts inside workload resource templates; test verification pattern for confirming metrics flow after start

### Modified Capabilities
- `kit-metrics-declaration`: Test verification requirement added — integration tests MUST assert that ≥10 metric series are visible in VictoriaMetrics within 30s of a successful `start`
