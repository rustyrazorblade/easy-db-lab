...
- `configuration/ClusterState.kt` — new `CniMode` enum; `InitConfig.ciliumEnabled: Boolean` replaced by `InitConfig.cni: CniMode = CniMode.Flannel`; `InitConfig.fromInit` maps from `--cni`
...
- Observability on a Cilium cluster: `cilium install` enables the agent (9962) and operator (9963) Prometheus endpoints and exposes Hubble UI as NodePort 31234; `OtelManifestBuilder` renders the `cilium-agent`, `hubble`, and `cilium-operator` scrape jobs only when `InitConfig.cni == Cilium`; `up` marks the install window with Grafana annotations tagged `cilium`; a read-only `platform cni` command reads `cilium-config` and the `CiliumNode` objects back
- `configuration/kubestatemetrics/KubeStateMetricsManifestBuilder.kt` — kube-state-metrics deployed with the core stack on every cluster, scraped by node-local pod discovery
