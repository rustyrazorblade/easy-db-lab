...
| AC | Topic matching is case-insensitive (`help Configs` == `help configs`) | `cli-help-topics: Topic matching is case-insensitive` | ✅ Covered |
...
| AC | The distribution ships ten seed topics (`provisioning`, `configs`, `kits`, `stress-testing`, `profiles`, `connecting`, `querying`, `observability`, `spark`, `cassandra`), each task-oriented, not a flag reference | `cli-help-topics: Each seed topic describes how to perform its operation` | ✅ Covered |
| AC | The `cassandra` topic is an umbrella database-management guide (lifecycle + version select/install) that points to `configs`/`stress-testing` | `cli-help-topics: Each seed topic describes how to perform its operation` (content review during implementation) | ✅ Covered |
