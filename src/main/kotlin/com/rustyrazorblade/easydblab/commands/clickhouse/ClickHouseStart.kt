 ...
 * - 's3_tier': Local disk as hot tier, S3 as cold tier (data moves automatically when local fills up)
 *
...
class ClickHouseStart : PicoBaseCommand() {
    ...
    override fun execute() {
        ...
        eventBus.emit(
            Event.ClickHouse.Deploying(shardCount, replicasPerShard, actualReplicas),
        )
    ...
    private fun waitForPodsIfRequired(controlNode: ClusterHost) {
        if (!skipWait) {
            eventBus.emit(Event.ClickHouse.WaitingPodsReady)
            k8sService
    ...
    private fun displayAccessInformation(
        ...
        eventBus.emit(
            Event.ClickHouse.DeploySuccess(
                bucket = bucket,
