...
- **`GrafanaDashboard`** — data class `(folder, jsonFileName)` for one core dashboard. Derives `stem`, `relativePath` (`<folder>/<file>`, the dashboard's place in the copied tree) and `resourcePath` (`/dashboards/<folder>/<file>` on the classpath). Nothing about a dashboard becomes a K8s name.
- **`GrafanaDashboardCatalog`** — runtime discovery. `discover()` scans the classpath under `dashboards/` with ClassGraph (same pattern as `InstallTemplateResolver` for kits) and yields one `GrafanaDashboard` per `<folder>/<name>.json`, sorted. Fails loudly if a JSON file is not exactly one directory deep or if `system-overview.json` (the home dashboard, `Constants.Grafana.HOME_DASHBOARD_STEM`) is absent. Registered as a Koin `single`; the folder name is the directory name verbatim. It is the only list of dashboards anywhere, and it is derived. Adding a dashboard = drop a JSON file into `dashboards/<folder>/`; adding a folder = make a directory.
- **`GrafanaDashboardTreeWriter`** — writes the catalog to a local directory as `<folder>/<file>.json`, copying each JSON from the classpath verbatim except `profiling.json`, whose `__PYROSCOPE_URL__` placeholder is replaced. No `TemplateService`: dashboards carry Grafana built-ins like `$__rate_interval` that general substitution corrupts. `services/GrafanaDashboardTreeUploader` writes that tree to a temp directory, uploads it with `RemoteOperationsService.uploadDirectory` to a `mktemp -d` staging directory under the SSH user's home on the control node, then swaps it into `GrafanaManifestBuilder.GRAFANA_DASHBOARD_HOST_PATH` (`/mnt/db1/grafana/dashboards`) with `sudo rm -rf && sudo mv && sudo chown -R 472:472`.
- **`GrafanaDashboardProvisioningConfig`** — typed (kotlinx.serialization + kaml) `dashboards.yaml` generation: exactly one file provider, `options.path` = `/var/lib/grafana/dashboards` and `foldersFromFilesStructure: true`, with no `folder`/`folderUid` (Grafana rejects that combination). Each directory of the copied tree becomes a Grafana folder of the same title, so kit dashboards installed into a folder of that name land beside the core ones.
- **`GrafanaManifestBuilder`** — builds the Grafana K8s resources (provisioning ConfigMap and Deployment) as typed Fabric8 objects. Nothing it builds varies with the catalog's contents; the catalog only supplies the home dashboard for `GF_DASHBOARDS_DEFAULT_HOME_DASHBOARD_PATH` (`/var/lib/grafana/dashboards/<folder>/<file>`). Uses `TemplateService` only for the cluster name. The Deployment mounts the `/mnt/db1/grafana` hostPath at `/var/lib/grafana`, which is how the copied tree reaches the pod, and includes a `grafana-image-renderer` sidecar container (port 8081) for server-side panel rendering. `DefaultGrafanaDashboardService.uploadDashboards` also deletes any ConfigMap labelled `grafana_dashboard=1`, which the earlier ConfigMap-per-dashboard delivery created and server-side apply would otherwise leave behind.
...
- **`OtelManifestBuilder`** — builds the main OTel Collector ServiceAccount, ClusterRole, ClusterRoleBinding, ConfigMap, and DaemonSet. Runs on all nodes, collects host metrics, Prometheus scrapes, file-based logs (system, Cassandra, ClickHouse), and OTLP. Uses `k8sattributes` processor to derive `node_role` from K8s node label `type` (db, app, control). RBAC grants read-only access to pods and nodes. Config uses OTel runtime env expansion (`${env:HOSTNAME}`), not `__KEY__` templates.
  - **Per-workload scrape jobs** come from the metrics registry as `WorkloadScrapeConfig`s. A config produces either a **static NodePort** job (`localhost:<port>`, every collector scrapes it, `instance` = collector hostname) or, when it carries a `podSelector`, a **pod service-discovery** job (`kubernetes_sd_configs`, role: pod). Pod-SD scrapes each matching pod directly on its pod IP + container port, filtered to pods on the collector's own node (`__meta_kubernetes_pod_node_name == ${env:HOSTNAME}`) so the DaemonSet doesn't produce duplicate series — `instance` becomes the pod name. Use pod-SD when one Service fronts multiple pods and you need per-pod attribution (e.g. TiKV per-store metrics); the kit declares `pod-selector` on its `scrape` metrics entry and sets `port` to the container port. See `buildPodScrapeJob`.
