...
- **`GrafanaDashboard`** — data class `(folder, jsonFileName)` for one core dashboard. Every K8s name and container path is derived from those two strings: `stem`, `configMapName` (`grafana-dashboard-<stem>`), `volumeName` (`dashboard-<stem>`), `folderPath` (`/var/lib/grafana/dashboards-<folder>`), `mountPath` (`<folderPath>/<stem>`), `resourcePath` (`/dashboards/<folder>/<file>`).
- **`GrafanaDashboardCatalog`** — runtime discovery. `discover()` scans the classpath under `dashboards/` with ClassGraph (same pattern as `InstallTemplateResolver` for kits) and yields one `GrafanaDashboard` per `<folder>/<name>.json`, sorted. Fails loudly if a JSON file is not exactly one directory deep, if a folder or stem is not a legal K8s name, or if `system-overview.json` (the home dashboard, `Constants.Grafana.HOME_DASHBOARD_STEM`) is absent. Registered as a Koin `single`; the folder name is the directory name verbatim. Adding a dashboard = drop a JSON file into `dashboards/<folder>/`; adding a folder = make a directory.
- **`GrafanaDashboardProvisioningConfig`** — typed (kotlinx.serialization + kaml) `dashboards.yaml` generation: one file provider per discovered folder, `folder` and `folderUid` both the directory name, `options.path` the folder's provider path. There is no root provider and `foldersFromFilesStructure` is deliberately unset.
- **`GrafanaManifestBuilder`** — builds all Grafana K8s resources (provisioning ConfigMap, dashboard ConfigMaps, Deployment) as typed Fabric8 objects from the catalog. Dashboard JSON is loaded directly from the classpath (no `TemplateService` — dashboards don't use `__KEY__` variables; only `profiling.json` gets `__PYROSCOPE_URL__` replaced). Uses `TemplateService` only for the cluster name. The Deployment includes a `grafana-image-renderer` sidecar container (port 8081) for server-side panel rendering.
