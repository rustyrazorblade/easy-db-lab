...
class InstallFromTest : BaseKoinTest() {
    ...
    private fun adHocSource(vararg files: Pair<String, String>): InstallTemplateResolver.TemplateSource.AdHoc {
    ...
    fun `renders template files substituting all standard variables`() {
        val source =
            adHocSource(
                "start.sh.template" to
                    """
                    #!/bin/bash
                    easy-db-lab platform create-pvs --workload __WORKLOAD_NAME__ --size __STORAGE_SIZE__
                    echo "DB nodes: __DB_NODE_COUNT__"
                    echo "Region: __REGION__"
                    """.trimIndent(),
            )

        Install().renderAndWrite(source = source, workloadName = "mydb", storageSize = "100Gi")
        ...
        assertThat(rendered).doesNotContain("__WORKLOAD_NAME__")
        assertThat(rendered).doesNotContain("__DB_NODE_COUNT__")
    }

    @Test
    fun `removes dot-template suffix from output file name`() {
        val source = adHocSource("values.yaml.template" to "workload: __WORKLOAD_NAME__")

        Install().renderAndWrite(source = source, workloadName = "mydb", storageSize = "50Gi")
    ...
    @Test
    fun `emits ArtifactWritten event for each rendered file`() {
        val source = adHocSource("start.sh.template" to "echo hi", "stop.sh.template" to "echo bye")

        Install().renderAndWrite(source = source, workloadName = "mydb", storageSize = "50Gi")

        val output = outputHandler.messages.joinToString("\n")
        assertThat(output).contains("Written:")
    }

    @Test
    fun `emits ScaffoldComplete event after rendering`() {
        val source = adHocSource("README.md.template" to "# __WORKLOAD_NAME__")

        Install().renderAndWrite(source = source, workloadName = "mydb", storageSize = "50Gi")

        val output = outputHandler.messages.joinToString("\n")
        assertThat(output).contains("Scaffold written to")
        assertThat(output).contains("start.sh")
    }
    ...
    fun `does not emit UnresolvedVariables when all placeholders are resolved`() {
        val source = adHocSource("start.sh.template" to "echo __WORKLOAD_NAME__")
