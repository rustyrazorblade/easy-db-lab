...
class MetricsLsTest : BaseKoinTest() {
    ...
    fun `displays empty message when no backups found`() {
        ...
        val command = MetricsLs()
        command.call()

        val output = outputHandler.messages.joinToString("\n")
        assertThat(output).contains("No VictoriaMetrics backups found")
    }
    ...
    fun `groups files by timestamp directory`() {
        val basePath =
            ClusterS3Path
                ...
                .resolve("clusters/test-cluster-test-id/victoriametrics")
        ...
        val output = outputHandler.messages.joinToString("\n")
        assertThat(output).contains("VictoriaMetrics backups:")
        assertThat(output).contains("20240101-120000")
        assertThat(output).contains("20240102-130000")
        assertThat(output).contains("2") // file count for first group
        assertThat(output).contains("1") // file count for second group
    }
}
