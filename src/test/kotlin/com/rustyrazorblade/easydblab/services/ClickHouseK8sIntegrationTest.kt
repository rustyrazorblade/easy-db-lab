...
class ClickHouseK8sIntegrationTest {
    ...
    fun setup() {
        ...
        // Create the clickhouse-cluster-topology ConfigMap that the StatefulSet depends on
        // This is normally created dynamically by ClickHouseStart command
        createClusterTopologyConfigMap()
    }

    /**
     * Creates the clickhouse-cluster-topology ConfigMap required by the ClickHouse server StatefulSet.
     * In production, this is created by the ClickHouseStart command.
     */
    private fun createClusterTopologyConfigMap() {
        val configMap =
            io.fabric8.kubernetes.api.model
                .ConfigMapBuilder()
                .withNewMetadata()
                .withName("clickhouse-cluster-topology")
                .withNamespace(DEFAULT_NAMESPACE)
                .addToLabels("app.kubernetes.io/name", "clickhouse-server")
                .endMetadata()
                .addToData("replicas-per-shard", "3")
                .build()
        client
            .configMaps()
    ...
    fun `should have created all expected ClickHouse resources`() {
        // Verify ConfigMaps (4 from manifests + 1 topology + kubernetes default)
        val configMaps =
            client
                .configMaps()
                .inNamespace(DEFAULT_NAMESPACE)
    ...
    fun `server statefulset should reference correct config volumes`() {
        ...
        assertThat(volumeNames)
            .withFailMessage("ClickHouse server should mount config, users, cluster-topology, and data volumes")
            .containsAll(listOf("config", "users", "cluster-topology", "data"))
