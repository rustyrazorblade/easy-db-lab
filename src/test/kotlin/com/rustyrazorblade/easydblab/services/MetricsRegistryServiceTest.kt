...
class MetricsRegistryServiceTest : BaseKoinTest() {
    ...
    fun `register creates ConfigMap with workload-metrics label`() {
        service.register(controlHost = controlHost, workloadName = "clickhouse", port = 9363, path = "/metrics")
    ...
    fun `register emits MetricsRegistered event`() {
        ...
        assertThat(events).anyMatch { it is Event.Workload.MetricsRegistered && it.workload == "clickhouse" && it.port == 9363 }
    }
    ...
    @Test
    fun `register emits OtelSynced event`() {
        val events = mutableListOf<Event>()
        eventBus.addListener(
            object : EventListener {
                override fun onEvent(envelope: EventEnvelope) {
        ...
        assertThat(events).anyMatch { it is Event.Workload.OtelSynced }
    }

    @Test
    fun `deregister deletes ConfigMap`() {
        service.deregister(controlHost = controlHost, workloadName = "clickhouse")
    ...
    fun `deregister emits MetricsDeregistered event`() {
        ...
        assertThat(events).anyMatch { it is Event.Workload.MetricsDeregistered && it.workload == "clickhouse" }
    }
    ...
    fun `deregister is a no-op when ConfigMap does not exist`() {
        ...
        val result = service.deregister(controlHost = controlHost, workloadName = "nonexistent")
    ...
    fun `register returns failure when ConfigMap creation fails`() {
        ...
        val result = service.register(controlHost = controlHost, workloadName = "clickhouse", port = 9363, path = "/metrics")
